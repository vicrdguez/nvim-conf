...
-- Wrapping everything on a func because plugin files are loaded in alphabetical order and heirline
-- was being loaded before the theme, thus it was not picking the override colors from the palette.
-- This function is then executed inside a autocmd that triggers with the `ColorScheme` event
--
local function setup_heirline()
  ...
  local Diagnostics = {
    ...
    static = {
      -- error_icon = function() return vim.fn.sign_getdefined('DiagnosticSignError')[1].text end,
      -- warn_icon = function() return vim.fn.sign_getdefined('DiagnosticSignWarn')[1].text end,
      -- info_icon = function() return vim.fn.sign_getdefined('DiagnosticSignInfo')[1].text end,
      -- hint_icon = function() return vim.fn.sign_getdefined('DiagnosticSignHint')[1].text end,
      error_icon = 'E ',
      warn_icon = 'W ',
      info_icon = 'I ',
      hint_icon = 'H ',
...
au('ColorScheme', {
  desc = 'Setup heirline when theme is loaded',
  pattern = 'catppuccin*',
  group = augroup('themes'),
